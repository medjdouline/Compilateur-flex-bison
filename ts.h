...
typedef struct element {
    ...
    char code[20];
    char type[20];
    char val[100]; // Valeur de la variable
    char parent[100];
    int size;
} element;
...
void add(char entite[], char code[], char type[], float val) {
    if(find(entite)) return; // Ne pas ajouter si l'entit� existe d�j�
    ...
    sprintf(temp->elt.val, "-"); // Initialiser la valeur � '-'
        ...
        if(tab[i] != NULL) {
            ...
            while(t != NULL) {
                if(strcmp(t->elt.type, "") != 0) {
                    printf("\t| %-15s | %-15s | %-15s | %-15s\n", t->elt.name, t->elt.code, t->elt.type, t->elt.val);
                }
                t = t->nxt;
            }
        }
...
int isNumber(char* str) {
    int i = 0, dotCount = 0, parensCount = 0;
    
    // Check for signed integers/floats within parentheses
    ...
    for (; str[i] != '\0'; i++) {
        ...
        // More than one dot means it's not a valid number
        if (dotCount > 1) {
            return 0;
        }
    } 

    // If parentheses were used, ensure they close properly
    if (parensCount) {
        if (str[strlen(str) - 1] != ')') {
            return 0;
        }
